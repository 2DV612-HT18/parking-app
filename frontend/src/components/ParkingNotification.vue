<template>
  <v-card class="elevation-12">
    <v-alert v-model="show" type="warning"> {{ notification }} </v-alert>
  </v-card>
</template>
...
export default {
  data: () => ({
    show: false,
    notification:
      "You are outside of your current parking area and might want to cancel your ticket.",
    ...
    geolocate() {
      ...
      navigator.geolocation.getCurrentPosition(position => {
        ...
        for (let i = 0; i < parkingAreas.length; i++) {
          ...
          console.log(areaLat);
          console.log(areaLng);
          this.calculatedDistance = this.distance(
            this.center.lat,
            this.center.lng,
            areaLat,
            areaLng,
          ...
          if (this.calculatedDistance > 100) {
            console.log(
              "Distance from parking area: " + this.calculatedDistance + " km"
            );
            console.log(
              "User is more than 100 km from current parking area – Show notification."
            );
            this.show = true;
            break;
          } else {
            ...
            console.log(
              "User is within 100 km of current parking area - No notification"
            );
            this.show = false;
          }
        }
